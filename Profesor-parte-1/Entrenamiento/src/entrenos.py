...
def lee_entrenos(fichero: str) -> list[Entreno]:
    ...
    '''
    entrenos = []  # quiero guardar varios entrena, no uno; con tuplas crearia una tupla nueva con las anteriores
    with open(fichero, 'r', encoding='utf-8') as f:
        lector = csv.reader(f)
        next(lector)  # Saltar la cabecera
        for fila in lector:
            tipo, fechahora, ubicacion, duracion, calorias, distancia, frecuencia, compartido = fila
            entreno = Entreno(   # Esto es una tupla
                tipo=tipo, # nombre campo en namedtuple=variable local
            ...
            entrenos.append(entreno)  # Guardo esa tupla en la lista
    return entrenos
