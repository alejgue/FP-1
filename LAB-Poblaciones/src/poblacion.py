...
def lee_poblaciones(ruta_fichero):
    with open(ruta_fichero, encoding='utf-8') as f:
        ...
        #res = []
        #for linea in f:
        #    res.append(RegistroPoblacion(*linea.strip().split(',')))
    #return res
...
def filtra_por_paises_y_anyo(poblaciones: list[RegistroPoblacion], anyo: int, paises: list[str]) -> list[tuple[str,int]]:
    conjunto_de_resultados = list()
    for linea in poblaciones:
        if (linea.pais in paises and linea.año==anyo):
...
def muestra_evolucion_poblacion(poblaciones, nombre_o_codigo):
    ...
    for linea in filtrado:
        ...
        poblacion.append(linea[0])
    return paises, poblacion
    genera_grafica_evolucion_poblacion("Evolucion de la pblacion de "+nombre_o_codigo, filtrado[0], filtrado[1])
...
if __name__ == '__main__':
    poblaciones = lee_poblaciones('..\\data\\population.csv') # cuidado con la ruta del terminal
    paises=calcula_paises(poblaciones)       # https://docs.python.org/es/3.9/library/csv.html   puede entrar examen
    ...
    print("España", spain, "Alemania", alemania)              